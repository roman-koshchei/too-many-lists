...
impl<T> LinkedList<T> {
    ...
    pub fn push_front(&mut self, value: T) {
        unsafe {
            let new_front = NonNull::new_unchecked(Box::into_raw(Box::new(Node {
                front: None,
                back: self.front, // not as in book, but i think it's fine
                value,
            })));
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        self.front.map(|front_ptr| unsafe {
            let front_node = Box::from_raw(front_ptr.as_ptr());

            self.front = front_node.back;
            if let Some(back) = front_node.back {
    ...
    pub fn front(&self) -> Option<&T> {
        unsafe { self.front.map(|node| &(*node.as_ptr()).value) }
    }
    pub fn front_mut(&mut self) -> Option<&mut T> {
        unsafe { self.front.map(|node| &mut (*node.as_ptr()).value) }
    }
    pub fn back(&self) -> Option<&T> {
        unsafe { self.back.map(|node| &(*node.as_ptr()).value) }
    }

    pub fn back_mut(&mut self) -> Option<&mut T> {
        unsafe { self.back.map(|node| &mut (*node.as_ptr()).value) }
    }
...
impl<'a, T> Iterator for Iter<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // it won't do the right for yielding the last valueent! That sort of
...
impl<'a, T> CursorMut<'a, T> {
    ...
    pub fn current(&mut self) -> Option<&mut T> {
        self.cur.map(|cur| unsafe { &mut (*cur.as_ptr()).value })
    }
